...
2. Verify the story matches **write-implementation-story**: all required `##` sections present; **Status** appropriate for pre-implementation (e.g. expect `Approved` or explicit user waiver if still `Draft`).
...
- Story body is expected to follow **write-implementation-story**.
