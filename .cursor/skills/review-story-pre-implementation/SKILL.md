...
8. **Dependencies and ordering**: Does **§5** match real sequencing risk? Relies on prior stories, migrations, or flags? If yes, are prerequisite stories **done** *and* are their **deliverables present in the working tree** (not only **`## 1. Status`** on the prerequisite file)? Flag wrong-branch / missing-merge risk. Call out any prerequisite implied elsewhere but **not** listed in **§5**.
...
- For each **prior story** cited in **§5** (or derived list), state whether implementers can proceed: cite prerequisite **`## 1. Status`** when relevant, and briefly note **evidence that prerequisite code/schema/config exists in the repo under review** (e.g. key file paths, migration presence)—**status alone is not enough** if the branch could be stale. Flag **unsafe ordering** when a prerequisite is not merged, not documented, or **marked done but deliverables missing from the tree**.
